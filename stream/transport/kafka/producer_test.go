...
func TestProducerError(t *testing.T) {
	opt := SinkOption{}
	opt.config = NewSaramaConfig()
	opt.retryCnt = SinkSendRetryCnt
	opt.retryInterval = SinkSendRetryInterval

	p, err := NewProducer([]string{"192.168.100.2:9092"}, cfg)
	...
	go func() {
		...
		defer close(done)
		for {
			promise := p.Emit("hainayun-qingcloud-test", "test", []byte{})
