...
func Init(serviceName string, metricsFactory metrics.Factory, logger log.Factory) opentracing.Tracer {
	...
	if err != nil {
		logger.Bg().Fatal("cannot parse Jaeger env vars", logf.Error(err))
	} else {
		cfg.ServiceName = serviceName
		logger.Bg().Info("start jaeger", logf.Any("cfg", cfg))
	}
	...
	if err != nil {
		logger.Bg().Fatal("cannot initialize Jaeger Tracer", logf.Error(err))
	}
	return tracer
}
